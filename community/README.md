...
[Scan control plane components of HyperShift hosted cluster using tailored CIS Benchmark for OpenShift profile](./CM-Configuration-Management/policy-compliance-operator-hypershift-hosted-cluster.yaml) | This example creates a `TailoredProfile` designed to scan control plane components of a HyperShift hosted cluster in addition to `ScanSettiing` and `ScanSettingBinding` to invoke a scan using the `TailoredProfile`. | The Compliance Operator can only scan OpenShift nodes. For more details, visit: [Understanding the Compliance Operator](https://docs.openshift.com/container-platform/latest/security/compliance_operator/compliance-operator-understanding.html).
