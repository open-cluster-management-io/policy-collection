...
[Policy to install RHODA Operator on OpenShift](./CM-Configuration-Management/policy-rhoda-operator.yaml) | This policy creates Red Hat OpenShift Database Access Operator on managed clusters and hub clusters allow users to onnect to managed databases fromtheir apps deployed in OpenShift.  | OpenShift 4.10 and above is required. Provider Accounts needs to be configured once operator has been installed successfully.
