...
[Policy to configure Metrics for RHACM-Observability-Stack](./CM-Configuration-Management/policy-observability-allowlist.yaml) | use this policy to configure the allowlist of metrics| Check its [documentation](https://github.com/open-cluster-management/multicluster-observability-operator/blob/main/manifests/base/config/metrics_allowlist.yaml) for more information
...
[Policy to install the Advanced Cluster Security Secure Cluster Services](./CM-Configuration-Management/policy-acs-operator-clusters.yaml) | Use this policy to install the Advanced Cluster Security operator to all OpenShift managed clusters and install the Secure Cluster Services to the stackrox namespace. | For more information on Advanced Cluster Security, visit [Red Hat Advanced Cluster Security for Kubernetes](https://www.openshift.com/products/kubernetes-security).  This policy requires policy template support available starting in Red Hat Advanced Cluster Management for kubernetes version 2.3
