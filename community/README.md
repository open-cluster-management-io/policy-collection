...
[Advanced Cluster Security Sensor policy](./CM-Configuration-Management/policy-check-acs-managed.yaml)| The `policy-check-acs-managed` policy checks for the Stackrox Sensor resources and proper configuration. The sensor is not installed by this policy. | For more information on Advanced Cluster Security, see the [Stackrox platform](https://www.stackrox.com/platform/).
